---
description: Jak správně vytvářet a nasazovat odlehčená (lightweight) API v monorepu na VPS
---
...
### Riziko a prevence OOM při příkazu `npm install` na rootu monorepa
Bohužel samotný root instalátor může lokálně alokovat hodně RAM (až 2GB), protože řeší stromy závislostí pro obrovské monorepo. 
- Abychom se pojistili proti OOM při spuštění `npm install` v Coolify, vyplatí se mít připraven velký **swap space** (minimálně 4GB na 8GB serveru). 
- Pokud to nepomůže, jediným finálním řešením je použití **Cloudflare Workers / Vercel Edge**, případně sjednocení všech skriptů do jedné jediné instalované aplikace `apps/master` jako *Monolitu*. 
